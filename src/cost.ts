...
function safeModelLabel(value: unknown): string {
  if (typeof value !== "string" || !value.trim() || UNSAFE_KEYS.has(value)) return "unknown";
...
function withLedgerLock<T>(fn: () => T): T | undefined {
  try { mkdirSync(STATE_DIR, { recursive: true, mode: 0o700 }); } catch { return undefined; }
  try { chmodSync(STATE_DIR, 0o700); } catch { /* best-effort */ }
  let fd: number | undefined;
  let ownerDevice: number | undefined;
  ...
  for (let attempt = 0; attempt < LOCK_ATTEMPTS; attempt++) {
    try {
      ...
      try {
        ...
        if (Date.now() - info.mtimeMs > LOCK_STALE_MS) unlinkSync(LOCK_FILE);
      } catch { /* another writer may be replacing the lock */ }
      try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 25); } catch { /* best-effort delay */ }
    }
  }
  ...
  try {
    ...
    try {
      ...
      if (currentLock.dev === ownerDevice && currentLock.ino === ownerInode) unlinkSync(LOCK_FILE);
    } catch { /* best-effort */ }
  }
}

function applyEntry(target: PersistedState, entry: TurnCost): void {
  ...
  if (target.recent.length > RECENT_LIMIT) target.recent.splice(0, target.recent.length - RECENT_LIMIT);
}

function writeStateFile(): boolean {
  const written = withLedgerLock(() => {
    let temporary: string | undefined;
    let fd: number | undefined;
    try {
      const merged = resetRequested ? emptyState() : load();
      for (const entry of pendingEntries) applyEntry(merged, entry);
...
function persist(): void {
  ...
  writeTimer = setTimeout(() => {
    writeTimer = null;
    if (!dirty) return;
    if (writeStateFile()) dirty = false;
  }, 250);
  if (typeof writeTimer.unref === "function") writeTimer.unref();
}

export function flushLedgerSync(): void {
  if (writeTimer) {
