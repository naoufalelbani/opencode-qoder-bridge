...
const sessionLeaseQueues = new Map();
...
async function withLeaseFile(lockPath, signal, fn) {
        ...
        try {
            ...
            try {
                ...
                if (info.isSymbolicLink())
                    throw new QoderSessionError("Invalid Qoder session lease");
                if (Date.now() - info.mtimeMs > SESSION_LEASE_STALE_MS)
                    await unlink(lockPath);
            }
            catch (lockError) {
                if (lockError instanceof QoderSessionError)
                    throw lockError;
            }
    ...
    if (!handle)
        throw new QoderSessionError("Could not acquire the Qoder session lease");
    ...
    catch { /* lock ownership is still represented by the open file */ }
 ...
 */
export function withQoderSessionLease(key, cwd, signal, fn) {
    if (!validKey(key))
        return Promise.reject(new QoderSessionError("Invalid Qoder session lease key"));
    ...
    const releaseAndClean = () => {
        release();
        if (sessionLeaseQueues.get(localKey) === current)
            sessionLeaseQueues.delete(localKey);
    };
...
async function withFileLock(fn) {
    ...
    for (let attempt = 0; attempt < LOCK_ATTEMPTS; attempt++) {
        try {
            ...
            try {
                ...
                if (info.isSymbolicLink())
                    throw new QoderSessionError("Invalid Qoder session lock");
    ...
    if (!handle)
        throw new QoderSessionError("Could not acquire the Qoder session store lock");
    try {
        return await fn();
    }
    finally {
...
function scopedStateKey(key, cwd) {
...
function recordForCwd(state, key, cwd) {
    ...
    if (scoped && normalizedCwd(scoped.cwd) === targetCwd)
        return { stateKey: scopedKey, record: scoped };
    ...
    if (legacy && normalizedCwd(legacy.cwd) === targetCwd)
        return { stateKey: key, record: legacy };
    return null;
}
...
async function load() {
    try {
        ...
        if (code === "ENOENT")
            return { state: emptyState(), writable: true };
    ...
    try {
        ...
        for (const [key, value] of Object.entries(parsed)) {
            if (validStateKey(key) && validRecord(value))
                state[key] = value;
            else
                valid = false;
        }
...
async function readResetEpoch() {
    try {
        ...
        if (!info.isFile() || info.isSymbolicLink() || info.size > MAX_RESET_BYTES)
            return "invalid";
...
async function writeResetEpoch(epoch) {
    let temporary;
    try {
        await mkdir(STATE_DIR, { recursive: true, mode: 0o700 });
        try {
...
export function getQoderSessionResetEpoch() {
    return readResetEpoch();
}
export async function getQoderSession(key, cwd) {
    if (!validKey(key))
        return null;
    return withLock(() => withFileLock(async () => {
        const { state } = await load();
        if (cwd)
            return recordForCwd(state, key, cwd)?.record ?? null;
        const legacy = Object.hasOwn(state, key) ? state[key] : undefined;
        if (legacy)
            return legacy;
...
export async function getQoderSessionForCwd(key, cwd) {
    return getQoderSession(key, cwd);
}
export function ensureQoderSession(key, qoderSessionId, cwd, expectedResetEpoch) {
    ...
    return withLock(() => withFileLock(async () => {
        ...
        if (existingMapping?.stateKey === key)
            delete state[key];
...
export function deleteQoderSession(key, cwd) {
    if (!validKey(key))
    ...
    return withLock(() => withFileLock(async () => {
        ...
        if (cwd) {
            ...
            if (mapping)
                delete state[mapping.stateKey];
            await save(state);
            return;
        }
        delete state[key];
        for (const stateKey of scopedKeys(state, key))
            delete state[stateKey];
        await save(state);
    }));
}
/** Delete a mapping after waiting for any active turn in the same workspace. */
export async function deleteQoderSessionForCwd(key, cwd, leaseKey = key) {
    ...
    try {
        ...
        if (completed !== true)
            throw new QoderSessionError("Could not reset the Qoder session while it was active");
    }
    finally {
