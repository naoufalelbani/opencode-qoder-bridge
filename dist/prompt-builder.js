    ...
    for (const item of output) {
        if (item && typeof item === "object" && "type" in item) {
            ...
            if (it.type === "text")
                ...
                parts.push(JSON.stringify(it.value, null, 2));
...
function imageFromPart(part) {
    ...
    const fromString = (raw, fallbackType) => {
        ...
        if (dataUrl) {
            if (!IMAGE_MEDIA_TYPES.has(dataUrl[1]) || dataUrl[2].length > MAX_BASE64_CHARS)
                return null;
        ...
        if (!IMAGE_MEDIA_TYPES.has(fallbackType) || raw.length > MAX_BASE64_CHARS)
            return null;
...
function trimToBudget(prompt, contextWindow) {
    ...
    const measure = (msgs) => approxTokens(msgs.map(serializeMessage).join("\n\n"));
    let dropped = 0;
    ...
    if (dropped > 0) {
        const marker = { role: "system", content: `<truncated_history count="${dropped}" />` };
