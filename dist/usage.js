 ...
 */
export function getLiveUsage(force = false) {
    const now = Date.now();
    ...
    const request = (async () => {
        ...
        try {
            ...
            const usage = await withTimeout(q.getUsageInfo(), FETCH_TIMEOUT_MS, `Qoder usage request exceeded ${FETCH_TIMEOUT_MS}ms`);
            if (usage) {
                cached = usage;
    ...
    inflight = request;
    request.then(() => { if (inflight === request)
        inflight = null; }, () => { if (inflight === request)
        inflight = null; });
    return request;
}
...
function bucketLine(label, b) {
    if (!b || typeof b.total !== "number" || !Number.isFinite(b.total) || b.total < 0)
        return null;
    const total = b.total;
...
export function formatUsageReport(u) {
    const lines = ["Qoder Account Usage"];
    const userType = safeLabel(u.userType);
    if (userType)
        lines.push(`  Plan: ${userType}`);
