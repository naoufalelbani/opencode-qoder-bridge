...
export function getQoderRegion(cliPath) {
...
export function regionPreference(environment = process.env) {
...
export function cliLoginHint(environment = process.env) {
...
function resolveCli(environment) {
    const preference = regionPreference(environment);
    const locations = describeSearchLocations().filter((location) => preference === "auto" || regionOfBase(location.base) === preference);
