...
function controlOptions(bridgeOptions, cwd, abortController) {
    ...
    if (closeGraceMs !== undefined)
        options.closeGraceMs = closeGraceMs;
    const cli = findQoderCLI();
    if (cli)
        options.pathToQoderCLIExecutable = cli;
    if (bridgeOptions.env)
        options.env = environment;
    if (bridgeOptions.proxy)
        options.proxy = bridgeOptions.proxy;
    if (bridgeOptions.vpcEndpoint)
        options.vpcEndpoint = bridgeOptions.vpcEndpoint;
    if (bridgeOptions.mcpServers && Object.keys(bridgeOptions.mcpServers).length > 0) {
        options.mcpServers = bridgeOptions.mcpServers;
    }
    if (bridgeOptions.extraArgs && Object.keys(bridgeOptions.extraArgs).length > 0) {
        options.extraArgs = Object.fromEntries(Object.entries(bridgeOptions.extraArgs).map(([key, value]) => [key.replace(/^--/, ""), value]));
    }
    return options;
}
/**
...
export async function openSdkControlSession(bridgeOptions, cwd) {
    ...
    try {
        ...
        try {
            activeQuery && await activeQuery.close();
        }
        catch { /* best effort */ }
        try {
            warm?.close();
...
export function formatMcpStatuses(statuses) {
    ...
    for (const status of statuses) {
        ...
        if (status.scope)
            details.push(`scope: ${status.scope}`);
        ...
        if (status.error)
            lines.push(`    Error: ${safeStatusText(status.error, "unknown", 1_024)}`);
    }
    return lines.join("\n");
}
function safeStatusText(value, fallback, maxLength = 512) {
    if (typeof value !== "string" || !value.trim())
        return fallback;
    return redactSensitiveText(value.replace(/[\u0000-\u001f\u007f-\u009f]/g, " ")).slice(0, maxLength) || fallback;
}
//# sourceMappingURL=sdk-control.js.map