...
function stringRecord(v: unknown): Record<string, string> | undefined {
  ...
  if (Object.keys(v).length > MAX_MCP_ENTRIES) return undefined;
...
function stringArray(v: unknown): string[] | undefined {
  ...
  if (v.length > MAX_MCP_ENTRIES) return undefined;
  const out: string[] = [];
  for (const item of v) {
    if (typeof item === "string" && item.length <= MAX_MCP_STRING_LENGTH && !CONTROL_CHARS.test(item)) out.push(item);
...
function timeoutValue(v: unknown): number | undefined {
...
function httpUrl(v: unknown): string | undefined {
  if (typeof v !== "string" || v.length > MAX_MCP_STRING_LENGTH || CONTROL_CHARS.test(v)) return undefined;
  try {
 ...
 */
function convertEntry(raw: unknown): McpServerConfig | null {
  if (!isRecord(raw) || raw.enabled === false) return null;
  if (raw.type !== undefined && raw.type !== "sdk" && raw.type !== "stdio" && raw.type !== "sse" && raw.type !== "http") return null;
  ...
  if (
    ...
    if (Array.isArray(raw.args) && !stringArray(raw.args)) return null;
 ...
 */
export function bridgeMcpServers(mcp: unknown): Record<string, McpServerConfig> {
  if (!isRecord(mcp)) return {};
  const out: Record<string, McpServerConfig> = {};
  let count = 0;
  for (const [name, raw] of Object.entries(mcp)) {
    if (count >= MAX_MCP_SERVERS) break;
    if (UNSAFE_KEYS.has(name) || name.length === 0 || name.length > 256 || CONTROL_CHARS.test(name)) continue;
