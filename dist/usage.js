...
export function formatUsageReport(u) {
    ...
    if (u.userType)
        lines.push(`  Plan: ${u.userType}`);
    if (typeof u.totalUsagePercentage === "number")
    ...
    if (u.expiresAt && u.expiresAt > 0)
        lines.push(`  Expires: ${new Date(u.expiresAt).toISOString().slice(0, 10)}`);
    return lines.join("\n");
}
