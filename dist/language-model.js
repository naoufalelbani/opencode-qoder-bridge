...
async function flushSdkBackgroundWork(activeQuery, bridgeOptions) {
    ...
    await Promise.all(operations.map(async ([label, operation]) => {
        try {
            await withTimeout(operation(), BACKGROUND_FLUSH_TIMEOUT_MS, `Qoder ${label} background work exceeded ${BACKGROUND_FLUSH_TIMEOUT_MS}ms`);
        }
        catch (error) {
...
export class QoderLanguageModel {
    ...
    buildQueryOptions(cli, sessionId, abortController, shouldResume, modelId = this.modelId, cwd = resolveCwd(this.bridgeOptions.cwd), onAuthExpired) {
        ...
        if (proxy)
            opts.proxy = proxy;
        if (this.bridgeOptions.memory)
            opts.memory = this.bridgeOptions.memory;
        if (this.bridgeOptions.securityScan)
            opts.securityScan = this.bridgeOptions.securityScan;
...
function normalizedToolInput(toolName, raw) {
    ...
    try {
        ...
        if (toolName === "bash") {
            ...
            if (commandMatch) {
                try {
                    return normalizeToolInputString(toolName, JSON.stringify({ command: JSON.parse(`"${commandMatch[1]}"`) }));
                }
                catch {
...
function debugInvalidToolInput(toolName, raw) {
